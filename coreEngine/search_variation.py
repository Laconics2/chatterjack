...
class Search():
    ...
    def searchKeyInf(self, questions):
        global inter, organName, className, personName, keywords, wrong
        ...
        if table == 'PERSON':
            searching = "SELECT " + inter + ", `sex`, `person_name`, `person_spe_name` FROM " + table + " WHERE `person_name` LIKE %s OR `person_spe_name` = %s;"
            try:
        ...
        if table == 'CLASS':
            searching = "SELECT " + inter + ", `class_name' FROM " + table + " WHERE `class_name` LIKE %s;"
            try:
                with self.__db.cursor() as cursor:
                    cursor.execute(searching, ('%' + cla + '%'))
                    keywords = cursor.fetchall()
                    return keywords
            except pymysql.Error as e:
                return False
