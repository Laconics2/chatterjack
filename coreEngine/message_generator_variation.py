...
class MessageGenerator():
    def generatePronouns(self):
        ...
        if keywords["sex"] is not "TBA":
            return keywords["sex"].split("/")
    ...
    def generateTimeString(self, whenString):
        ...
        possibleStartPhrase = {"This class is held on ",
                              f"{keywords["class_name"]} has these meeting times:",
    ...
    def generate_answer(self, questions):
        ...
        if table == 'PERSON':
            ...
            if nameType:
                ...
                if speOrFullName:
                    if "person_name" in keywords.keys():
                        personName = keywords["person_name"]
                    else:
                        personName = keywords['person_spe_name']
                if '_where' in keywords.keys():
                    answer = personName+"'s office is located in  " + keywords['_where'] + '.'
                if '_who' in keywords.keys():
                    answer = personName + " is the " + keywords['_who'] + '.'
                if '_when' in keywords.keys():
                    answer =  personName + " office hour is at " + keywords['_when'] + '.'

            else:
                pronouns = self.generatePronouns()

                if '_where' in keywords.keys():
                    answer = pronouns[2]+" office is located in  " + keywords['_where'] + '.'
                if '_who' in keywords.keys():
                    if pronouns[0].lower() is not "They":
                        answer = pronouns[0]+ " is the " + keywords['_who'] + '.'
                    else:
                        answer = pronouns[0] + " are the " + keywords['_who'] + '.'
                if '_when' in keywords.keys():
                answer =  pronouns[2] + " office hour is at " + keywords['_when'] + '.'
        ...
        if table == 'CLASS':
            ...
            if '_when' in keywords.keys():
                answer = self.generateTimeString(keywords["_when"])
