...
from coreEngine.coredata import interrogatives, nauInf, calls
    ...
    def expandPerson(self, questions):
        ...
        for named in words_low:
            if named in calls:
                ...
                if words_org[spePerInd] == '.':
                    spePerInd = words_low.index(named) + 2
    ...
    def classes(self, questions):
        ...
        for match in re.finditer(class_exp, doc.text):
            ...
            if course is not None:
                # there is the space between the course prefix and course number
                if re.match(cla_numer, course.text):
                    words_low = self.segmentList(questions)
                    if course.text in words_low:
                        # combine the course prefix and course number together
                        i = words_low.index(course.text)
                        combineCourse = words_low[i-1] + course.text
                    searchInf[combineCourse] = 'CLASS'
                # no space between the course prefix and course number
                else:
...
print(answer.intention(questions))
"""