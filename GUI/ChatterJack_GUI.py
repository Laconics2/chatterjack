...
from GUI import speech_to_text, record_user_speech, text_to_speech
...
class ChatterJackFrame(wx.Frame):
    ...
    def __init__(self, parent_app, title):
        ...
        # show the background
        try:
            image = wx.Image("smile.png",
                             wx.BITMAP_TYPE_ANY).ConvertToBitmap()
            image = wx.Bitmap(image.ConvertToImage().Scale(
                400, 400, wx.IMAGE_QUALITY_HIGH))
            self.bitmap = wx.StaticBitmap(panel, -1, image, pos=(100, 100))
        except IOError:
            raise SystemExit
        ...
        self.txt_box = wx.TextCtrl(panel, 2, value="Ask Your Question Here...", size=((wx.DisplaySize()[1]/1.5), -1),
